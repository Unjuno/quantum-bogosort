...
The purpose is adversarial: before assigning any specifically Everettian meaning to observer accessibility, ask how much of the first-person law can already be represented by ordinary classical ascertainment, record multiplicity, or selection conditioning, including across multiple policies or experimental contexts.
...
| `c` | policy/context index | none | element of an index set `C` | index |
| `mu_c` | base law in context `c` | none | probability measure on common measurable space | measure |
| `Q_c` | observer-conditioned target law in context `c` | none | `Q_c << mu_c` | measure |
| `r_c` | `dQ_c/dmu_c` | none | nonnegative, `E_{mu_c}[r_c]=1` | Radon–Nikodym density |
| `a_c` | classical context-specific selection weight | dimensionless by convention | nonnegative, finite positive mean | function |
| `a` | context-invariant classical selection weight | dimensionless by convention | same function used in all contexts | function |
...
## A testable restricted null: context-invariant selection

The no-go above is intentionally strong because it grants the classical null the same freedom to change its selection rule across contexts that an unconstrained accessibility model has. A useful experiment must restrict that freedom for an independent reason.
...
Hence for any two contexts `c,d`, wherever the densities are defined and positive,
...
which is constant in `omega` `lambda`-almost everywhere.
...
`lambda`-almost everywhere for every context, then the family is generated by that context-invariant selection function.
...
Taking the ratio for two contexts gives the stated constant. The converse follows by substitution and the fact that each `Q_c` is already normalized. `QED`

### Falsification rule for the restricted null
...
then **no context-invariant classical selection function `a(omega)` can generate both selected laws** under the stated common-support assumptions.

This does not identify an Everett mechanism by itself. It only falsifies that particular restricted classical null. A more flexible context-dependent classical selection model can still fit the data unless additional restrictions are independently justified.
...
This context-indexed boundary is consistent with the broader sample-selection and missing-data literature: nonrandom selection mechanisms are generally not identified from the selected distribution without additional assumptions, exclusion restrictions, instruments, parametric structure, or other cross-context information.
...
10. **Shared-null test:** the constant density-ratio condition requires a common support/reference measure so `r_c/r_d` is meaningfully comparable. Failure of this condition falsifies only the context-invariant selection null, not every classical selection model.
11. **Policy dependence:** allowing arbitrary `S_pi` in the QBS model while forbidding policy dependence in the classical null would create an asymmetric comparison. Identifying restrictions must be independently motivated and applied fairly.
12. **Everett boundary:** none of these classical representations derives or refutes an Everett accessibility mechanism. They establish stringent nulls and show what extra cross-context structure a specifically Everettian interpretation must supply.
