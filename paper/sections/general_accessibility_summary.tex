...
The clean S2 result assumes that accessibility is a deterministic function of the predictive score. That assumption is useful for isolating the basic mechanism, but it is stronger than necessary. For the general result in this section, assume $U$ and $S$ are square-integrable, with $S\ge0$ and $0<\mathbb E[S]<\infty$. Let
\[
...
For the normalized explained-variance interpretation, additionally assume
\[
\operatorname{Var}(U)>0,
\qquad
\operatorname{Var}(S)>0.
\]
Define the fractions of total outcome and accessibility variance explained by their conditional means,
\[
A_U
=
...
\sqrt{(1-A_U)(1-A_S)}
\]
guarantees positive outcome--accessibility covariance. The right-hand side is nonnegative, so this worst-case certificate necessarily requires positive conditional-mean correlation. Under perfectly aligned conditional means, $\rho_{ma}=1$, it simplifies to
\[
A_U+A_S>1.
\]
