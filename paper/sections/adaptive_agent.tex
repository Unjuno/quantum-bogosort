...
The same population condition can be certified from independent held-out data. Corollary S2.5 assumes known bounds
\[
|Y_t|\le B_Y,
\qquad
0\le S_t\le B_S,
...
|U_T-Y_t|\le B_R,
\]
and applies simultaneous Hoeffding bounds to the empirical moments of $Y_t$, $S_t$, $Y_tS_t$, $S_t^2$, and $(U_T-Y_t)^2$. It constructs a lower confidence bound $D_L$ satisfying
\[
P\!\left(
\operatorname{Cov}(U_T,S_t)\ge D_L
\right)
\ge1-\delta.
\]
Thus an independently evaluated sample with
\[
D_L>0
\]
certifies positive population covariance at confidence at least $1-\delta$.

Corollary S2.6 formalizes the training/evaluation split: an arbitrarily complex learner may be trained and selected upstream, and the S2.5 guarantee remains valid after conditioning on the entire training procedure provided the final certification sample is independent and the post-training population bounds are valid. Corollary S2.7 further permits choosing among a finite family of candidates on the same held-out sample by constructing simultaneous certificates. With equal family-wise allocation across $K$ predeclared candidates, the Hoeffding radius becomes
\[
\tau_{n,\delta,K}
=
\sqrt{\frac{\log(10K/\delta)}{2n}},
\]
and whichever candidate is selected from the held-out data retains the simultaneous lower-bound guarantee. Uncorrected best-of-$K$ reporting and post-hoc invention of new candidates are not covered.
...
\rightarrow
\text{held-out finite-sample certificate}
\rightarrow
\text{selection-safe certification}.
\]

Experiment E2 tests the minimal version of the predictive mechanism in a nonlinear environment. A misspecified model that cannot represent the relevant interaction fails to produce predictive alignment, while a small interaction-capable model succeeds. E3 supplies a paired endogenous-policy example. These simulations support the premise that learning can generate useful predictive ordering in toy environments; S2--S2.7 prove increasingly operational covariance implications once calibration, score-measurable accessibility, boundedness, held-out evaluation, and selection accounting are specified. None of these steps establishes a physical Everettian accessibility map.
