...
D_L>0
\]
certifies positive population covariance at confidence at least $1-\delta$. The theorem requires the predictor and accessibility rule to be fixed relative to the held-out sample; reusing training or tuning data requires separate adaptivity accounting.
...
\rightarrow
\text{independent held-out finite-sample certificate}.
\]

Experiment E2 tests the minimal version of the predictive mechanism in a nonlinear environment. A misspecified model that cannot represent the relevant interaction fails to produce predictive alignment, while a small interaction-capable model succeeds. E3 supplies a paired endogenous-policy example. These simulations support the premise that learning can generate useful predictive ordering in toy environments; S2--S2.5 prove increasingly operational covariance implications once calibration, score-measurable accessibility, and the stated statistical assumptions are specified. None of these steps establishes a physical Everettian accessibility map.
