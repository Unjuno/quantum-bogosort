...
\begin{corollary}[Posterior-mean self-calibration]
Let $B$ be an internal information state and suppose
\[
Y=\mathbb E[U\mid B].
\]
Then
...
\mathbb E[U\mid Y]=Y.
\]
Consequently, if $S=s(Y)$ with $s$ nondecreasing, then
\[
\operatorname{Cov}(U,S)
=
...
\ge0.
\]
If $Y$ is nonconstant and $s$ is strictly increasing on the essential range of $Y$, the covariance is strictly positive.
\end{corollary}

\begin{proof}
Because $Y=\mathbb E[U\mid B]$ is measurable with respect to $B$,
\[
\mathbb E[U\mid Y]
=
\mathbb E[\mathbb E[U\mid B]\mid Y]
=
\mathbb E[Y\mid Y]
=Y.
\]
The covariance conclusion then follows from S2 with $m(Y)=Y$.
\end{proof}

\begin{corollary}[Approximate-calibration robustness]
Assume $U$, $Y$, and $S=s(Y)$ are square-integrable and define
\[
e(Y)=\mathbb E[U\mid Y]-Y.
\]
Then
\[
\operatorname{Cov}(U,S)
...
Write
\[
\mathbb E[U\mid Y]=Y+e(Y).
\]
Using the S2 projection identity and covariance linearity,
\[
\operatorname{Cov}(U,S)
=
\operatorname{Cov}(Y,S)
...
\sqrt{\operatorname{Var}(e(Y))\operatorname{Var}(S)}.
\]
Taking the worst-case negative sign for the calibration-error term gives the stated lower bound.
\end{proof}

\begin{corollary}[Prediction-MSE certificate]
Under the square-integrability assumptions of the preceding corollary,
\[
\operatorname{Cov}(U,S)
\ge
\operatorname{Cov}(Y,S)
...
\mathbb E[e(Y)^2].
\]
Substituting this upper bound into the approximate-calibration robustness inequality proves the result.
\end{proof}

The prediction MSE admits the exact conditional-variance decomposition
\[
\mathbb E[(U-Y)^2]
=
...
\mathbb E[e(Y)^2].
\]
Thus the prediction-MSE certificate is conservative: ordinary MSE includes irreducible conditional outcome variance in addition to calibration error. The previous calibration-error bound is sharper when $e(Y)$ can be estimated directly.
...
The theorem and corollaries are sufficient-condition results about an abstract accessibility map. Posterior-mean self-calibration is exact only when the score equals the true conditional expectation under the model being analyzed; an approximate learned predictor is covered through the calibration-error and prediction-MSE bounds. None of these results derives Everettian observer self-location from quantum dynamics.
