...
This appendix collects the complete measure-theoretic proofs used by the manuscript. Throughout, $S\ge 0$, $0<\mathbb E[S]<\infty$, and all displayed expectations are assumed finite whenever required.

\subsection{Covariance identity}

\begin{theorem}[QBS covariance identity]
For any random variable $X$ such that $\mathbb E[|X|S]<\infty$,
\[
...
Z=\mathbf 1_{\{X\ge c\}}.
\]
Then $\mathbb E[Z]=P(X\ge c)$ and $\mathbb E_{\mathrm{FP}}[Z]=P_{\mathrm{FP}}(X\ge c)$.
\end{proof}
...
Let recognition state $R\in\{0,1\}$ induce policy-dependent pairs $(U_R,S_R)$ and define
\[
V_R
=
