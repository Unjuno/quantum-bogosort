...
The branch-coherence plot separates cross-copy action-correlation change from single-observer first-person gain. Figure 6 reads the current paired reproduction output. The current E5 rho-sweep field is named `action_corr_increment`; older locked historical summaries retain their historical schema and are not rewritten.

## Reproduction

Use the repository's recorded Python environment (`.python-version`) and pinned dependencies in `requirements.txt`. From the repository root:
...
The scripts now contain executable scientific regression guards in addition to deterministic seeds. These guards check the experiment-specific identities, nulls, counterexample/sign behavior, predictive-alignment controls, and branch-coherence contrasts described in the H/T/D/C/U cards. GitHub Actions then byte-compares all twelve current E1–E5 reproduction CSVs against the committed outputs.

Important reproducibility details:

- E1 enforces the configured monotone-FOSD direction, independence null, and nonmonotone CDF-crossing controls.
- E2 checks the interaction-capable versus misspecified/random predictive-alignment separation and the expected degradation with increasing noise.
- E3 uses paired primitive randomness for recognition-off/on comparisons and enforces both decomposition closure and the recognition-label null.
- E4 regenerates and checks both the fixed-selector sign identity and the general selector-map-shift decomposition.
- E5 reuses the same primitive random arrays across every execution-strength value and every shared-environment correlation value, enforces the exact `q=0` null, and checks the shared-versus-independent recognition contrast at matched marginal prevalence.

See [`../figures/README.md`](../figures/README.md) for figure provenance and regeneration details, and [`../docs/pre_announcement_execution_audit_2026-08-19.md`](../docs/pre_announcement_execution_audit_2026-08-19.md) for the commit-fixed execution audit that motivated the strengthened checks.
