...
The manuscript appendix contains complete proofs of T1–T5 and the established supplementary derivations. The post-v0.2 development branch adds an isolated proof appendix for S2 so it can be reviewed without changing the core five theorem set.
...
S2 proves the exact projection identity:
...
All five experiments are rerun by GitHub Actions. S2 does not add a sixth core experiment: E2 and E3 remain the mechanism demonstrations for the theorem's calibration premise.
...
| Adaptation necessarily generates that ordering | NOT CLAIMED; E2/E3 SIMULATION-SUPPORTED IN TOY MODELS |
...
2. Derive learning-theoretic sufficient conditions under which adaptation makes `E[U_T|Y_t=y]` correctly ordered in `y`; S2 now proves the subsequent covariance step.
3. Extend S2 to accessibility with residual randomness by controlling the conditional-covariance term in the law of total covariance.
