...
### Context-indexed extension

The same non-identifiability persists across arbitrary families of contexts if the classical null may retune selection separately in each context.

Let `c` index policy, intervention, or experimental context. For each `c`, let `mu_c` be a base law and let `Q_c << mu_c` be a target observer-conditioned law. Define
...
has record-size-biased law exactly equal to `Q_c`. Thus observing multiple selected laws does not identify a mechanism if a competing null is free to choose an unrelated selection channel in every context.

A restricted context-invariant selection null assumes one nonnegative measurable `a(omega)` shared across contexts:
...
If the base laws are equivalent to a common reference measure, then
...
and therefore for any pair of contexts `c,d`,
...
is constant almost everywhere on the common positive-density support. Conversely, a family satisfying `r_c=a/Z_c` for one common `a` is generated by that shared selection rule; normalization forces `Z_c=E_{mu_c}[a]`.

If the same base law `mu` applies in every context, this proportionality strengthens to
...
under the shared latent selection null.

These are structural restrictions, not automatic empirical tests. Operational use additionally requires the base and observer-conditioned laws to be identifiable on a common observable state space. If the relevant state is latent and only a context-dependent projection `X_c=T_c(omega)` is observed, latent density-ratio violations can disappear after projection, while observed distribution differences can arise from the changing trajectory/observation map even under a shared latent selector.

The complete proof/stress audit is [`context_identifiability_audit_2026-08-23.md`](context_identifiability_audit_2026-08-23.md).
...
- a latent cross-context density-ratio restriction is empirically testable without identifying the relevant base/selected laws after observation or projection;
...
- latent cross-context violations can disappear after projection, and observed distribution changes can arise from context-dependent trajectory/observation maps even under shared latent selection;
