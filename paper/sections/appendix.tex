...
Let $N_B$ be a finite nonnegative integer-valued adverse-trigger count and define
\[
S=\lambda^{N_B},
\qquad
V(\lambda)
...
\]
For the downweighting interpretation take $0<\lambda\le1$. Since $N_B$ is finite and nonnegative, $0<\lambda^{N_B}\le1$ almost surely and therefore
\[
0<\mathbb E[\lambda^{N_B}]\le1.
\]
At each evaluation point used to define the weighted value, assume
\[
\mathbb E[|U|\lambda^{N_B}]<\infty,
\]
so the numerator is absolutely integrable and $V(\lambda)$ is finite and well-defined.

For the derivative identity at a fixed positive $\lambda_0$, assume $V$ is considered on an open positive neighborhood of $\lambda_0$ and that differentiation may be interchanged with numerator and denominator expectations there. In particular, in addition to the weighted-value condition above, require the relevant weighted derivative quantities
\[
\mathbb E[|U|N_B\lambda^{N_B}],
\qquad
\mathbb E[N_B\lambda^{N_B}]
\]
to be finite at the evaluation point, together with a valid domination condition in a neighborhood. Define the $\lambda$-weighted expectation by
\[
\mathbb E_\lambda[X]
=
...
\]
Indeed, writing $N(\lambda)=\mathbb E[U\lambda^{N_B}]$ and $D(\lambda)=\mathbb E[\lambda^{N_B}]$, differentiation under the expectation and the quotient rule give
\[
...
\]
which is the weighted covariance. If the modeled family is restricted to $\lambda\le1$, the corresponding statement at $\lambda=1$ is the left derivative under the same dominated-differentiation conditions from below.

\subsection{Gaussian minimal model}

Let $(L,Y)$ be jointly standard normal with correlation $-1\le\rho\le1$. Let
\[
S
=
\begin{cases}
...
\]
Under the toy parameterization
\[
\lambda=1-q(1-\alpha),
\qquad
0\le q\le1,
...
0\le\alpha\le1,
\]
we have $0\le\lambda\le1$ and
\[
\mathbb E_{\mathrm{FP}}[L]
=
\frac{2q(1-\alpha)\rho}{[2-q(1-\alpha)]\sqrt{2\pi}}.
\]
The denominator is strictly positive under the stated parameter domain. This is an exact result for the Gaussian toy model, not a physical Everett prediction.
